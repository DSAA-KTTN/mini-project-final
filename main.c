...
void on_button21_clicked(GtkButton *b){
    ...
    gtk_label_set_text(GTK_LABEL(label23),"This word already exists!");
    }
}
...
void on_entry31_activate(){
	...
	if (btseln(dictionary, engword, vietword, dsize, &rsize) != 0)
	{
		...
		{	
			...
			if (strcmp(a, b) == 0 )
			{	
				strcpy(cacheStorage[i].completeText, dictWord);
				i++;
			}
		...
		if(i!=0) {
			for(int j=0;j<i;j++){
                if(j>=10) break;
            gtk_button_set_label(button31[j],cacheStorage[j].completeText);
            }
		}else{
...
void on_entry31_move_cursor(){
    char tmpe[256];
     ...
     if(strcmp(array[0].completeText,"") != 0){
        ...
        for(int i=1;i<11;i++){
                if(strcmp(array[0].completeText, "") == 0) break;
